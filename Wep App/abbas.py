...
from video import colorize_video
...
@app.route('/outputV', methods=['POST'])
def outputV():
    file_name = './static/input.mp4'
    out_name = './static/out.mp4'
    f = request.files.get('file_video')
    if f:
        f.save(file_name)
    else:
        return jsonify({'error': 'Please Choose File'})
    colorize_video(file_name, out_name)
    #os.remove(file_name)
    return jsonify({'input_path':file_name, 'img_path': out_name})
