...
def colorize_img(frame):
    ...
    img_bgr_out = np.clip(255 * cv2.cvtColor(img_lab_out, cv2.COLOR_Lab2BGR), 0, 255).astype(np.uint8)
    return img_bgr_out[:, :, ::-1]


def colorize_video(file_name, out_name):
    clip1 = VideoFileClip(file_name)#.subclip(start,end)
    video_clip = clip1.fl_image(lambda img: colorize(img)) #NOTE: this function expects color images!!
    video_clip.write_videofile(out_name, audio=True)
